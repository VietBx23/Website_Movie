...
@WebServlet({ "/ReportServlet", "/ReportServlet/Create", "/ReportServlet/Update", "/ReportServlet/Delete",
		"/ReportServlet/Edit", "/ReportServlet/Reset", "/ReportServlet/select", "/ReportServlet/selectTitle" })
public class ReportServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (uri.contains("selectTitle")) {
			selectOptionTitleShare(req, resp);
			req.setAttribute("users", user);
...
//				.getResultList();
	...
	public void selectOptionTitleShare(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {

		try {
//			String jpql = "select o from Video o where o.favorites IS EMPTY";
			String titleVideo = req.getParameter("titleVideo");
			StoredProcedureQuery query = em.createNamedStoredProcedureQuery("ReportShareVideo.spShareFriend");
			query.setParameter("title", "%" + titleVideo + "%");
			System.out.println(titleVideo);
			List<Object[]> listVideo = query.getResultList();
